...
Class Template {
    ...
    function render($dict) {//est clase convierte las palabras en valores 
        $this->set_dict($dict);
        return str_replace(array_keys($dict), array_values($dict), $this->template);//retorna las palabras en valores
    }

    function render_regex($dict = array(), $id = '') {
        $regex = "#<!--$id-->([\s\S]*)<!--$id-->#";//expresion regular
